...
export default function DateRangePicker({
  ...
  const openCalendar = (target: CalendarTarget) => {
    ...
    const selected =
      target === 'start' ? parseISODate(startValue) : parseISODate(endValue);
  ...
  return (
    <div ref={calendarRef} className="relative flex items-end gap-2">
      <div>
      ...
      <span className="pb-2.5 text-sm text-ink/25">–</span>
      ...
      {showDuration && duration > 0 && (
        <span className="mb-1.5 rounded-full border border-[#F0E5DE] bg-[#FFF8F4] px-2 py-1 text-[11px] font-semibold tabular-nums text-[#F36A21]">
          {duration}d
        </span>
      )}
        ...
        >
          ...
          <div className="grid grid-cols-7 px-4 pb-4">
            {calendarDays.map(({ date, currentMonth }, index) => {
              ...
              const rangeMiddle = inRange && !selectedStart && !selectedEnd;
              const disabled =
                calendarTarget === 'end' &&
                !!startDate &&
                isBeforeDay(date, startDate);
