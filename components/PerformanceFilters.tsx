...
import { PerformanceFilters as PerformanceFiltersType } from '@/lib/performanceTypes';
...
interface Props {
  ...
  restaurants: string[];
  onChange: (filters: PerformanceFiltersType) => void;
}

export default function PerformanceFilters({ filters, restaurants, onChange }: Props) {
  ...
  const allRestaurants = useMemo(
    () => restaurants.map((r) => r.trim()).filter(Boolean),
    [restaurants]
  );

  const committedRestaurants = useMemo(() => {
    if (!filters.restaurant || filters.restaurant === 'All') return allRestaurants;
    return filters.restaurant
      .split('|')
      .map((r) => r.trim())
      .filter(Boolean)
      .filter((r) => allRestaurants.includes(r));
  }, [filters.restaurant, allRestaurants]);

  const filteredRestaurants = useMemo(() => {
    const query = search.trim().toLowerCase();
    if (!query) return allRestaurants;
    return allRestaurants.filter((r) => r.toLowerCase().includes(query));
  ...
  const isAllSelected =
    ...
    allRestaurants.every((r) => draftRestaurants.includes(r));

  const toggleRestaurant = (restaurant: string) => {
    setDraftRestaurants((prev) => {
      const exists = prev.includes(restaurant);
      if (prev.length === allRestaurants.length && allRestaurants.length > 1 && exists) {
        return [restaurant];
      }
      if (exists) return prev.filter((r) => r !== restaurant);
      return [...prev, restaurant];
    });
  };

  const toggleAll = () => {
    setDraftRestaurants(isAllSelected ? [] : [...allRestaurants]);
  };
  ...
  const applyRestaurantSelection = () => {
    ...
    if (draftRestaurants.length === allRestaurants.length) {
      onChange({ ...filters, restaurant: 'All' });
    } else {
      onChange({ ...filters, restaurant: draftRestaurants.join('|') });
    }
    setRestaurantOpen(false);
    setSearch('');
  };
  ...
  const displayText =
    committedRestaurants.length === allRestaurants.length && allRestaurants.length > 0
      ? 'All restaurants'
  ...
  const setCompareEnabled = (enabled: boolean) => {
    ...
    onChange({
      ...filters,
      compareEnabled: true,
      compareMode: filters.compareMode === 'custom' ? 'custom' : 'previous',
      compareStartDate: filters.compareMode === 'custom' ? filters.compareStartDate || previous.startDate : previous.startDate,
      compareEndDate: filters.compareMode === 'custom' ? filters.compareEndDate || previous.endDate : previous.endDate,
    });
  };
  ...
  return (
    <div className="w-full">
      <div className="flex w-full flex-col gap-5 lg:flex-row lg:items-stretch">
        {/* RESTAURANT */}
        <div ref={restaurantRef} className="relative min-w-0 flex-1">
          <FilterSectionTitle number="01" title="Restaurant" />

          <button
            type="button"
            onClick={() => (restaurantOpen ? (setRestaurantOpen(false), setSearch('')) : openRestaurants())}
          ...
          >
            <span className="flex min-w-0 items-center gap-3">
              <RestaurantIcon />
              <span className="truncate text-[14px] font-medium text-[#34363A]">{displayText}</span>
            </span>
            <ChevronDown rotated={restaurantOpen} />
          </button>
            ...
            <div className="absolute left-0 top-[calc(100%+8px)] z-[300] w-full min-w-[340px] overflow-hidden rounded-[16px] border border-[#E7E2DE] bg-white shadow-[0_18px_50px_rgba(25,20,15,0.16)]">
              ...
              >
                <span
                  className={`flex h-[22px] w-[22px] shrink-0 items-center justify-center rounded-[7px] border transition ${
                    isAllSelected ? 'border-[#F36A21] bg-[#F36A21]' : 'border-[#BDB8B4] bg-white'
                  }`}
                >
                  {isAllSelected && <CheckIcon />}
                </span>
                <span className="text-[14px] font-semibold text-[#171717]">Select all</span>
              ...
              <div className="max-h-[330px] overflow-y-auto">
                {filteredRestaurants.length === 0 ? (
                  <div className="px-4 py-8 text-center text-[13px] text-[#999]">No outlets found</div>
                ) : (
                  filteredRestaurants.map((restaurant) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {selected && <span className="h-[7px] w-[7px] shrink-0 rounded-full bg-[#F36A21]" />}
                      </button>
                    );
                  })
                )}
              ...
              <div className="flex items-center justify-between border-t border-[#ECE8E5] bg-[#FCFBFA] px-4 py-3">
                <span className="text-[11px] font-medium text-[#8A8581]">
                  {draftRestaurants.length} {draftRestaurants.length === 1 ? 'outlet' : 'outlets'} selected
                </span>
                <button
                  type="button"
        ...
        <div className="min-w-0 flex-[1.35]">
          <FilterSectionTitle number="02" title="Date range" />
          <div className="mt-3">
            <DateRangePicker
      ...
      <div className="mt-8 rounded-[14px] border border-[#F3DDD0] bg-gradient-to-r from-[#FFFDFC] via-[#FFFBF9] to-[#FFFDFC] px-5 py-5 shadow-[0_5px_20px_rgba(243,106,33,0.04)]">
        <div className="flex w-full flex-col gap-5 xl:flex-row xl:items-center">
          <div className="flex min-w-[235px] items-center gap-3 xl:border-r xl:border-[#F0E5DF] xl:pr-7">
            <div>
              <div className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-[#85888D]">Compare</div>
              <button
                type="button"
                role="switch"
                ...
                className={`relative flex h-[30px] w-[50px] items-center rounded-full transition ${
                  filters.compareEnabled ? 'bg-[#F36A21]' : 'bg-[#D9D4D0]'
                }`}
              >
                <span
                  className={`absolute top-[3px] h-[24px] w-[24px] rounded-full bg-white shadow-[0_2px_5px_rgba(0,0,0,0.12)] transition ${
                    filters.compareEnabled ? 'left-[23px]' : 'left-[3px]'
                  }`}
                />
              </button>
            </div>
            <div className="pt-[22px]">
              <div className="text-[13px] font-medium text-[#F36A21]">Enable comparison</div>
              <div className="mt-0.5 whitespace-nowrap text-[12px] text-[#9A9591]">Compare with a previous period</div>
            </div>
          </div>
          ...
          {filters.compareEnabled && (
            <>
              <div className="min-w-[255px] xl:px-1">
                <div className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-[#85888D]">Comparison period</div>
                <div className="flex h-[48px] items-center rounded-[10px] border border-[#E8E0DB] bg-white p-[4px]">
                  <button
                    ...
                    className={`h-[40px] flex-1 rounded-[8px] px-3 text-[12px] font-medium transition ${
                      filters.compareMode === 'previous' ? 'bg-[#FFF0E8] text-[#E96A2C]' : 'text-[#85888D] hover:text-[#E96A2C]'
                    }`}
                  >
                    Previous period
                  </button>
                  <button
                    type="button"
                    onClick={() => onChange({ ...filters, compareMode: 'custom' })}
                    className={`h-[40px] flex-1 rounded-[8px] px-3 text-[12px] font-medium transition ${
                      filters.compareMode === 'custom' ? 'bg-[#FFF0E8] text-[#E96A2C]' : 'text-[#85888D] hover:text-[#E96A2C]'
                    }`}
                  >
                    Custom range
                  </button>
              ...
              <div className="min-w-0 flex-1 xl:border-l xl:border-[#F0E5DF] xl:pl-7">
                <div className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-[#85888D]">Compare dates</div>
                <DateRangePicker
                  ...
                  onChange={(compareStartDate, compareEndDate) =>
                    onChange({ ...filters, compareMode: 'custom', compareStartDate, compareEndDate })
                  }
                  onOpen={() => setRestaurantOpen(false)}
                  closeSignal={calendarCloseSignal}
                />
...
function FilterSectionTitle({ number, title }: { number: string; title: string }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="text-[13px] font-bold uppercase tracking-[0.01em] text-[#3D4148]">{title}</span>
    </div>
  );
}
...
function RestaurantIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" className="shrink-0 text-[#F36A21]">
      <path
      ...
      />
      <path d="M5 15V20" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
      <path d="M19 15V20" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
      <path d="M8 4V9" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
      <path d="M16 4V9" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
    </svg>
  );
}
...
function SearchIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="shrink-0 text-[#666]">
...
function ChevronDown({ rotated = false }: { rotated?: boolean }) {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
      fill="none"
      className={`shrink-0 text-[#555] transition-transform ${rotated ? 'rotate-180' : ''}`}
    >
